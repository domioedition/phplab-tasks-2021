...
class Strings implements StringsInterface {
     ...
     * @return string
     */
    public function snakeCaseToCamelCase(string $input):
        string {
         ...
         * @return string
         */
        public function mirrorMultibyteString(string $input):
            string {
                $multiString = explode(" ", $input);
                $result = [];
                foreach ($multiString as $word) {
                    preg_match_all('/./us', $word, $box);
                    $word = implode('', array_reverse($box[0]));
             ...
             * @return string
             */
            public function getBrandName(string $noun):
                string {
